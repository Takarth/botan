...
std::vector<Test::Result> test_secret_derivation_rfc8448_rtt1()
   ...
   return Test::flatten_result_lists(
      ...
      CHECK_both("ticket nonce counter counts", [&](Cipher_State* cs, Connection_Side, Test::Result& result)
         {
         result.test_is_eq("nonce is 0x00, 0x00", cs->next_ticket_nonce(), Botan::TLS::Ticket_Nonce({0x00, 0x00}));
         result.test_is_eq("nonce is 0x00, 0x01", cs->next_ticket_nonce(), Botan::TLS::Ticket_Nonce({0x00, 0x01}));
         result.test_is_eq("nonce is 0x00, 0x02", cs->next_ticket_nonce(), Botan::TLS::Ticket_Nonce({0x00, 0x02}));
      ...
      CHECK_both("PSK", [&](Cipher_State* cs, Connection_Side, Test::Result &result) {
         ...
         const auto psk = cs->psk(Botan::TLS::Ticket_Nonce({0x00, 0x00}) /* ticket_nonce as defined in RFC 8448 */);
