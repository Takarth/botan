...
class DSA_Signature_Operation final : public PK_Ops::Signature_with_EMSA
   {
   public:
      DSA_Signature_Operation(std::shared_ptr<DL_PrivateKey> key,
      ...
      size_t max_input_bits() const override { return m_key->group().q_bits(); }

      secure_vector<uint8_t> raw_sign(const uint8_t msg[], size_t msg_len,
                                      RandomNumberGenerator& rng) override;
   private:
      std::shared_ptr<DL_PrivateKey> m_key;
      BigInt m_b, m_b_inv;
   };
...
class DSA_Verification_Operation final : public PK_Ops::Verification_with_EMSA
   {
   public:
      DSA_Verification_Operation(std::shared_ptr<DL_PublicKey> key,
                                 const std::string& emsa) :
   ...
   private:
      std::shared_ptr<DL_PublicKey> m_key;
   };
