...
PSK::PSK(TLS_Data_Reader& reader,
   ...
   if(message_type == Handshake_Type::ServerHello)
      ...
      if(psks.empty())
         {
         throw TLS_Exception(Alert::DECODE_ERROR, "Empty PSK list");
         }
      ...
      if(binders_length == 0)
         {
         throw TLS_Exception(Alert::DECODE_ERROR, "Empty PSK binders list");
         }
      ...
      for(auto& psk : psks)
         ...
         if(!reader.has_remaining() || reader.read_so_far() - binders_offset >= binders_length)
            {
            throw TLS_Exception(Alert::ILLEGAL_PARAMETER, "Not enough PSK binders");
            }
      ...
      if(reader.read_so_far() - binders_offset != binders_length)
         {
         throw TLS_Exception(Alert::ILLEGAL_PARAMETER, "Too many PSK binders");
         }
...
PSK::PSK(std::pair<Session, Session_Handle>& session_to_resume, Callbacks& callbacks)
   ...
   const auto cipher = session.ciphersuite();
   ...
   cpsk.emplace_back(Client_PSK
      {
      .ticket = Ticket(handle.opaque_handle(), age, session.session_age_add()),
...
std::unique_ptr<PSK> PSK::select_offered_psk(const Ciphersuite& cipher,
   ...
   if(auto selection = session_mgr.choose_from_offered_tickets(tickets, cipher.prf_algo(), callbacks, policy))
      ...
      if(session.ciphersuite().prf_algo() != cipher.prf_algo())
         {
         throw TLS_Exception(Alert::INTERNAL_ERROR, "Application chose a ticket that is not compatible with the negotiated ciphersuite");
         }
